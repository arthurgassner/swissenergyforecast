...
def test__query_load_and_forecast__future_ts(entsoe_client: ENTSOEClient):
    ...
    # when
    fetched_df = entsoe_client._query_load_and_forecast(start_ts=future_ts, end_ts=future_ts + timedelta(hours=24))
...
def test__query_load_and_forecast__specitic_ts(entsoe_client: ENTSOEClient):
    """Querying the ENTSO-E API with a timestamp whose load/forecast is known."""

    # given - when
    fetched_df = entsoe_client._query_load_and_forecast(
